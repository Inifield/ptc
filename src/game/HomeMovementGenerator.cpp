...
void HomeMovementGenerator<Creature>::_setTargetLocation(Creature& owner)
{
    ...
    init.MoveTo(x, y, z, MMAP::MMapFactory::IsPathfindingEnabled(owner.GetMapId()), true);
    init.SetWalk(false);
    init.Launch();

    arrived = false;

    owner.ClearUnitState(uint32(UNIT_STATE_ALL_STATE & ~(UNIT_STATE_POSSESSED | UNIT_STATE_EVADE | UNIT_STATE_IGNORE_PATHFINDING)));
}
