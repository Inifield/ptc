...
void SmartAI::GenerateWayPointArray(Movement::PointsArray* points)
{
    ...
    if (me->m_movementInfo.HasMovementFlag((MovementFlags)(MOVEMENTFLAG_CAN_FLY)))
    {
...
bool SmartAI::IsEscortInvokerInRange()
{
    ...
    if (targets)
    {
        float checkDist = me->GetInstanceData() ? SMART_ESCORT_MAX_PLAYER_DIST*2 : SMART_ESCORT_MAX_PLAYER_DIST;
        if (targets->size() == 1 && GetScript()->IsPlayer((*targets->begin())))
        {
            Player* player = (*targets->begin())->ToPlayer();
...
void SmartAI::MoveInLineOfSight(Unit* who)
{
    ...
    // @todo: Made use canAttack instead of CreatureAI::MoveInlineOfSight(who) if is possible!
    // The function is pretty different at WotLK client, feel free to check the structures of SunwellCore or TrinityCore2 ;]
    // if (me->canAttack(who))
    //     AttackStart(who);
...
void SmartGameObjectAI::SpellHit(Unit* unit, const SpellEntry* spellInfo)
{
    GetScript()->ProcessEventsFor(SMART_EVENT_SPELLHIT, unit, 0, 0, false, spellInfo);
}
